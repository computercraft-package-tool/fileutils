...
function fileutils.storeFile(filepath,content)
...
function fileutils.readFile(filepath,createnew)
	local readfile = fs.open(filepath,"r")
	if not readfile then
		if not (createnew==nil) then
			fileutils.storeFile(filepath,createnew)
			return createnew
		else
			return false
		end
...
function fileutils.storeData(filepath,data)
	fileutils.storeFile(filepath,textutils.serialize(data):gsub("\n",""))
end
...
function fileutils.readData(filepath,createnew)
	if createnew then
		return textutils.unserialize(fileutils.readFile(filepath,textutils.serialize({}):gsub("\n","")))
	else
		local content = fileutils.readFile(filepath,nil)
